...
- **자가 관찰·자가 치유 RPA — 작업방식 전환(2026-07-20)**: 개발 환경에서 실제 gov 사이트는 조직 정책 차단(프록시 403 CONNECT — 정부24·복지로·대법원 전부)이라 개발자가 실화면을 직접 볼 수 없다. 그래서 '스크린샷 반복 → 추측 개발 → 실기기 오류'의 눈감고 루프를 끊는 두 축을 도입: ① **자가 관찰**(`rpa/diagnostics.py`) — 실패 시 그 순간 실화면의 접근성 트리·프레임·마커·URL을 **PII 없이**(값 미수집·실Chromium 계약 테스트) 파일 하나로 저장. gov24 주소선택 실패·doc/apply 최상위 예외에 `_dump_diag` 배선(어떤 예상 못 한 실패든 구조 남김). `GET /api/_diagnostics/latest`(본인 PC 전용·공유 403)로 최신본 반환, `AgentStatusStrip [진단 복사]`가 기술정보(_diag)+실패 화면 구조를 한 번에 클립보드 복사 → 사용자는 스샷 대신 **이 한 번의 붙여넣기**로 실화면 구조 전달. ② **자가 치유** — 발급 폼이 '진행 신호 없음'으로 정체하면 사람 개입 전에 필수 select 재선택(결정론)+ai_fill LLM 재계획(키 있을 때·빈 칸만·값일치·타인칸 가드)으로 스스로 보정·재시도. 안 되면 기존 사용자-수정 유예 폴백으로 이어짐. 검증: 실Chromium e2e로 '값 채운 폼도 값 무유출' 계약+엔드포인트 공유게이트 pytest 락.
