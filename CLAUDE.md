...
- **기능**: 3D 카툰 히어로(지연 마운트), 프로필 위저드+분석, **자연어 한 문장 즉시 분석**(`QuickAsk`/`parseQuery`, 음성 포함),
  ...
  **도메인 검색**(`lib/search.ts`: 생활어→행정용어 동의어 17군 + 개념단위 관련도 랭킹 — '노인 일자리' 다단어·'전세→주거' 생활어 매칭),
  정책 탐색(검색·정렬·금액필터·**지역(시도)필터**·증분렌더, **지자체 지역배지**), 나의 복지(관심·상태관리·혜택계산·비교·**사후관리 모니터링**),
  서류 준비 도우미, **신청 키트**(정책 상세에 공식 신청 딥링크 + 내 정보 미리채움 복사·인라인 입력, `ApplyKit`/`prefill.ts`),
...
- **품질 게이트(모두 통과)**: `npm run lint`(eslint9 flat, react-hooks, 0건) · `npm test`(vitest **119**) ·
