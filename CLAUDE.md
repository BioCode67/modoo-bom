...
- **품질 게이트(모두 통과)**: `npm run lint`(eslint9 flat, react-hooks, 0건) · `npm test`(vitest **224**) ·
  `tsc --noEmit` · `npm run build` · E2E 스모크(`frontend/e2e/smoke.py`, 실브라우저 4여정) / 백엔드 `pytest`(13). 변경마다 브라우저 회귀 검증.
