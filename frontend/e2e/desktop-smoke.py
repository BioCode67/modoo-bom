...
검증 27종(번호는 추가 순서 — 실행 순서는 코드 순):
  1 상태 스트립 · 2 진단 복사(PII 0) · 2.5 발급 전 점검(6항목·서류함 무결성) · 3 서류함 등록+첨부 후보 배지 · 3.5 🖍 가리기
  4 자동첨부 미리보기 · 5 개별 삭제(2탭) · 5.5 유효기간 배지+📦 ZIP · 5.55 👁 파일 열람 · 5.6 종류별 그룹핑(최신본 대표) · 5.65 🧹 이전 버전 정리 · 5.7 부족분만 발급→📨 자동신청만 · 5.8 자유 선택 일괄발급 · 5.85 🔁 손상 원탭 재발급 · 5.87 🔎 앱 내 실측 확인 · 5.88 후보 칩·자동신청 실측 정직 실패 · 5.9 빈 상태 슬림 모드 · 5.95 🩺 조용한 자가점검
...
def main() -> int:
    ...
    if issues:
        ...
        for i in issues[:10]:
            print("  ", i)
        return 1
    print("✅ 데스크탑 기능 27종 + pageerror 0 — 전부 통과")
    return 0
