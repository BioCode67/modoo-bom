...
export const COMBO_RULES: ComboRule[] = [
  {
    // 기초급여만 택1(65세에 전환)이고 부가급여는 병급이라 '하나만'(exclusive)은 오도 → 'reduction'으로.
    // 장애인연금을 포기하면 부가급여(월 9만원)를 잃으므로, 신청을 포기하지 않게 안내한다(취약계층 금전손실 방지).
    a: 'POL-001', b: 'POL-003', kind: 'reduction', // 기초연금 ↔ 장애인연금(기초급여 택1 · 부가급여 병급)
    note: '만 65세가 되면 장애인연금 “기초급여”는 기초연금으로 전환돼 이 둘 중 하나만 받아요. 하지만 장애인연금 “부가급여”(월 9만원)는 기초연금과 함께 계속 받으니, 장애인연금 신청을 포기하지 마세요.',
  ...
  {
    a: 'POL-003', b: 'POL-002', kind: 'reduction', // 장애인연금(기초급여) → 생계급여 소득산입
    note: '생계급여를 받는 중증장애인은 장애인연금 기초급여가 소득으로 잡혀 생계급여가 그만큼 줄 수 있어요. 다만 부가급여는 소득 산정에서 빠져 온전히 받아요. 둘 다 신청은 가능해요.',
    source: '보건복지부 국민기초생활보장 사업안내(2026)',
  },
  {
