...
function isAnnual(renewal: string): boolean {
  ...
  const r = renewal || ''
  // '연 1회(매년 재확인)'·명시적 매년 신호를 1회성 게이트보다 '먼저' 판정 — '연 1회'가 '1회'로 오검출돼 죽던 것(감사 확정)
  // '재선정·재계약'(자활근로 '연간 재선정'·주거 '연 단위 재계약')도 매년 갱신 — bare '연간'은 상시운영이라 제외 유지(monitoring.ts와 동일)
  if (/매년|연\s*1회|재확인|재산정|재판정|재선정|재계약/.test(r)) return true
  // 여기 도달=1회성/기간형만 — 1회성 바우처('1회 (1년 이내 사용)')를 매년 갱신으로 오판 방지
  if (/1회|사용|유효|만기|소진|이내|일시/.test(r)) return false
  // '연간 운영·서비스 제공'(상시 프로그램)은 매년 재확인이 아님 → 바(bare) '연간'은 제외. 명시적 주기만.
