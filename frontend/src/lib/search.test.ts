...
describe('실사용 질의 회귀(2026-07 품질 스윕)', () => {
  ...
  it('감사 회귀: 1글자 질의 "암" — 이름에 "암" 든 정책이 동의어(재활·간병비) 오답보다 위', () => {
    // 사용자가 직접 친 1글자 원문 단어는 매칭 허용(동의어 확장의 1글자 제외와 구분).
    const AMB = [
      P('A-1', '암환자 의료비 지원', '의료', '암환자', '', '치료비 지원'),
      P('A-2', '장애인 직업재활시설 생산장려금', '장애', '재활시설', '', '재활 지원'),
    ]
    const r = searchPolicies(AMB, '암')
    expect(r.length).toBeGreaterThan(0)
    expect(r[0].id).toBe('A-1') // '암' 이름 매칭이 '재활' 동의어 매칭보다 상위
  })
  it('2차 회귀 방지: 1글자 "집"은 합성어 중간 매칭 금지 — 어린이집·추모의집이 상위 아님', () => {
    // '암' 예외가 '집'→'어린이집' 오탐을 재도입하지 않도록: 1글자 사용자어는 이름 '시작/경계'만 인정.
    const HOME = [
      P('H-1', '청년월세 특별지원', '주거', '무주택 청년', '', '월세 20만원'),
      P('H-2', '보육료 지원(어린이집)', '보육', '영유아', '', '어린이집 보육료'),
      P('H-3', '무주추모의집 운영', '기타', '', '', '장례시설'),
    ]
    const r = searchPolicies(HOME, '집')
    // '집'은 주거 동의어군(집구하기)으로 확장 → 주거 정책이 상위, 어린이집/추모의집은 배제
    expect(r[0]?.id).toBe('H-1')
    expect(r.some((p) => p.id === 'H-2' || p.id === 'H-3')).toBe(false)
  })
  it('2차 회귀 방지: 1글자 라틴/숫자는 카탈로그를 무관하게 쓸어담지 않음', () => {
    const r = searchPolicies(cat, 'a')
    // 'a'는 한글 정책명 시작과 무관 → 대량 오답 금지(경계 매칭만 인정)
    expect(r.length).toBeLessThan(20)
  })
  it('띄어쓰기 변형이 정책명 그대로여도 최상위 — "청년 월세 지원"↔"청년월세지원"', () => {
    // 한국어 복합어 띄어쓰기 요동으로 '거의 이름 그대로'인 질의가 이름 가산을 놓쳐 밀리던 문제.
    const HS = [
      P('T-9', '청년월세지원', '청년', '독립 거주 청년', '중위 60% 이하', '월 최대 20만원'),
      P('T-H1', '경기 청년 임대주택 (GH 모집공고)', '청년주거', '무주택 청년', '', '임대'),
      P('T-H2', '청년 전세임대주택', '청년주거', '무주택 청년', '', '전세임대'),
    ]
