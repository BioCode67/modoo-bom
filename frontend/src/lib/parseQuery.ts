...
export function parseProfileFromText(text: string): UserProfile {
  ...
  else if (!incomeNegated && /저소득|(?:소득|월급|월수입|벌이|수입|연봉).*(없|적|낮|많지\s*않|많진\s*않|많이\s*부족|넉넉[하지]*\s*않|부족)|형편.*(어렵|어려|힘들|안\s*좋|나쁨|막막)|생활(?:비)?.*(어렵|어려|힘들|빠듯|쪼들)|살기.*(어렵|어려|힘들|막막)|돈.*(없|부족|막막)|가난|빈곤|생활고|끼니.*(거르|거를|걸러|굶)|장사.*안\s*(돼|되|굴러)|매출.*(줄|없|감소|반토막)|폐업|가게.*(접|문\s*닫)/.test(t)) p.income_percentile = 40
