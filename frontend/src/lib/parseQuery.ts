...
export function parseProfileFromText(text: string): UserProfile {
  ...
  // 고소득 부정 가드: '여유가 있진 않아요'·'잘 사는 편은 아니에요'를 '여유.*있'·'잘 사'로 고소득 오분류해
  //   저소득 정책서 배제하던 것 방지(감사 4차 #7). 부정·어려움 신호가 근처면 고소득 분기를 막는다.
  const richNegated = /(여유|잘\s*사).{0,7}(않|없|아니|어렵|힘[들드]|못)/.test(t)
  ...
  // 자영업 폐업(가게 접음)은 '폐업' 생애이벤트로 기록 — 엔진의 폐업자 구제(실업급여·국민취업 demographicMismatch 예외
  //   + checkPolicyDoc jobless)가 자연어 경로에서도 발동하게. (기존엔 '폐업'을 저소득 신호로만 소비해 구제가 사문화됐음, 감사 확정)
  if (/폐업|가게.*(접었|접고|접어|문\s*닫|정리했)|사업.*(접었|접고|정리했|망)|장사.*(접었|접고|망)|문\s*닫았/.test(t)) {
    if (!p.life_events.includes('폐업')) p.life_events.push('폐업')
  }
