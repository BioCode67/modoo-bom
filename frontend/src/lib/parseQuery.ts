...
export function parseProfileFromText(text: string): UserProfile {
  ...
  else if (!incomeNegated && /저소득|(?:소득|월급|월수입|벌이|수입|연봉).*(없|적|낮|많지\s*않|많진\s*않|많이\s*부족|넉넉[하지]*\s*않|부족|시원찮|신통찮|쥐꼬리)|형편.*(어렵|어려|힘들|안\s*좋|나쁨|막막)|(?:생활(?:비)?|생계).*(어렵|어려|힘들|빠듯|쪼들|막막|곤란)|살기.*(어렵|어려|힘들|막막)|돈.*(없|부족|막막)|가난|빈곤|생활고|끼니.*(거르|거를|걸러|굶)|입에\s*풀칠|장사.*안\s*(돼|되|굴러)|매출.*(줄|없|감소|반토막)|폐업|가게.*(접|문\s*닫)/.test(t)) p.income_percentile = 40
  ...
  if (/암(이|에|을|으로|\s|$)|백혈병|투병|입원(했|중|하)|큰\s*병|수술\s*받|수술비|희귀질환|난치병|중증질환|지병|만성질환|병원비|치료비|(아이|애(?!인)|아들|딸|아기).*아[파프픈팠픕]|(몸|건강).{0,4}(아[파프픈팠픕]|안\s*좋|나[빠쁨])|아파서\s*(일|근로|출근|생계|움직)/.test(t) && !p.life_events.includes('질병')) {
