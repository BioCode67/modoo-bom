...
export function parseProfileFromText(text: string): UserProfile {
  ...
  //   ⚠️ '세(?!대)': "3세대가 함께 살아요"의 'N세대'(가구 구성 표현)를 3세 신청자로 오파싱하지 않음 —
  //     붙여 쓴 '세대'만 제외하므로 공백 있는 "30세 대학생"(정상 나이)은 그대로 명시 나이(감사 회귀수정).
  //   ⚠️ 룩어헤드에 필러(짜리·배기 등) 허용: "5살배기/5살짜리 아이"의 자녀 나이가 신청자(부모) 나이로
  //     오귀속되고 _ageExplicit까지 켜져 되물음 방어선이 꺼지던 것 차단 — 필러는 자녀 명사가 뒤따를 때만
  //     제외 사유가 되므로 "40살 정도 됐어요"(본인 나이)는 여전히 잡힌다(감사 회귀수정, 아래 kids 절과 통일).
  const exact = tForAge.match(/(\d{1,3})\s*(?:세(?!대)|살)(?!\s*(?:짜리|배기|정도|가량|쯤|밖에)?\s*(?:아이|자녀|아들|딸|아기|아동|손주|손자|손녀))/)
  ...
  //   '셋(?![집방값])': 사이시옷 표기 '전셋집·월셋집·셋방·전셋값'의 '셋'이 명수로 오인돼 자녀 1명 가구를
  //   다자녀로 오분류하던 같은 계열 함정 차단 — '아이 셋'(진짜 명수)은 그대로(감사 회귀수정).
  if (/다자녀|아이.*(셋(?![집방값])|3\s*명|세\s*명)|자녀.*(셋(?![집방값])|3\s*명|세\s*명)/.test(t)) p.household_type = '다자녀가구'
  ...
  // 나이-먼저 자녀 표기 — "5살 아이" 및 필러가 낀 "5살짜리/배기/정도 아이", 손주 표기까지.
  //   '배기' 누락 시 "5살배기 아이"의 자녀 나이(5)가 유실돼 기본 3세로 뭉개지고, 위 나이 절이 5살을
  //   부모 나이로 잡던 결함 — 필러 목록은 나이 절 룩어헤드와 동일하게 유지한다(감사 회귀수정).
  const kids = [...t.matchAll(/(\d{1,2})\s*(살|세)\s*(?:짜리|배기|정도|가량|쯤|밖에)?\s*(아이|자녀|아들|딸|아기|손주|손자|손녀)/g)]
