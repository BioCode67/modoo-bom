...
describe('isCashBenefit / sumCashMonthly', () => {
  ...
  it('무료 교육/훈련은 조건부 별도수당이 있어도 현금 아님(청년 취업사관학교)', () => {
    // 이 사업은 무료 교육이고 '월 50만원'은 국민취업지원제도(별도 제도) 대상자만 → 이 사업의 현금으로 표기하면 과장
    expect(isCashBenefit('교육비(수강료) 전액 무료(예치금 수료 후 환급). 훈련수당은 국민취업지원제도 대상자만 월 50만원(별도 제도)')).toBe(false)
  })
