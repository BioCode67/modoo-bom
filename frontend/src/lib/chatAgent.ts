...
// 개인화 의도만 로컬 eligibilityReply로. 바단어 '자격/추천/대상'을 그대로 두면 특정 정책 지식질문
// ('기초연금 자격이 뭐야')까지 가로채 개인화 답변(또는 프로필 요구)으로 오라우팅됨 → 개인화 앵커(내가/나/저/제)나
// 요청동사를 요구하고, 나머지는 searchReply(그리고 백엔드 있으면 클라우드 LLM)로 흘려보낸다(감사 확정).
const ELIG_RE = /(내가|나|저|제).*(받|대상|자격|해당)|받을\s*수\s*있|혜택.*뭐|뭐.*받|추천.*(해|줘|부탁)|복지\s*추천|추천\s*복지|어떤.*복지|맞는.*복지/
