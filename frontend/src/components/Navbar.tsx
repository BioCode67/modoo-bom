          ...
          <nav className="ml-4 hidden md:flex items-center gap-1" aria-label="주요 메뉴">
            {NAV.map(({ view: v, label, icon: Icon }) => (
              ...
              >
                ...
                {label}
                {v === 'my' && savedCount > 0 && (
                  <span className="ml-0.5 rounded-full bg-peach-700 text-white text-[10px] font-bold px-1.5 py-0.5 leading-none">
                    {savedCount}
                  </span>
                )}
              </button>
        ...
        <div className="grid grid-cols-4">
          {NAV.map(({ view: v, label, icon: Icon }) => (
            ...
            >
              ...
              {label}
              {v === 'my' && savedCount > 0 && (
                <span className="absolute top-1.5 right-[22%] rounded-full bg-peach-700 text-white text-[9px] font-bold px-1 leading-tight">
                  {savedCount}
                </span>
              )}
            </button>
