      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="card-cute p-6 sm:p-8 bg-gradient-to-br from-sprout-50 via-white to-sky2-50 relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            ...
            {/* '다시 분석'은 나머지 보조 버튼과 같은 흰 배경이라 묻혔다(실사용 피드백) →
                연초록 채움+진한 테두리로 한 단계만 강조. 단색 초록(btn-primary)은 실제 주행동인
                '나의 복지에서 관리'와 위계가 꼬여 피한다. */}
            <button
              onClick={onReset}
              className="btn-secondary !py-2.5 !bg-sprout-100 !border-sprout-400 !text-sprout-900 !font-extrabold hover:!bg-sprout-200 hover:!border-sprout-500"
            >
              <RotateCcw className="h-4 w-4" /> {helperMode ? '도우미 종료' : '다시 분석'}
            </button>
