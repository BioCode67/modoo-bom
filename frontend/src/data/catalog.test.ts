...
// 시드 = 정부 큐레이션(POL-) + 민간재단 큐레이션(PRV-) + 청년주택 공고(HOU-). nameKey는 공백+괄호 무시.
const SEED = new Set([...WELFARE_POLICIES, ...PRIVATE_POLICIES, ...HOUSING_POLICIES].map((p) => nameKey(p.name))).size
