...
def _income_ceiling(doc: str):
    ...
    for m in re.finditer(r"하위\s*([0-9]{2,3})\s*%", applicant_doc):
        ...
        if ceil is None or v > ceil:
            ceil = v
    # %가 안 적힌 자산심사형('차상위 이하'·'수급 가구' 등)도 저소득 상한(중위 50% 근사)으로 본다.
    #   단, 아래 '우대 선정'은 하드 소득요건이 아니라 상한으로 보면 51~100%ile 자격자가 부당 배제된다(프론트 패리티):
    #   ① '(기초생활수급자·차상위 우선)'처럼 우선/우대로 끝나는 괄호군(그룹 전체가 우대) ② 괄호 밖 '차상위/수급자 우선·우대'
    #   ③ '미수급자'(부정어 — 소득 무관). 이 셋을 먼저 걷어낸 뒤 차상위/수급 여부를 판정한다.
    if ceil is None:
        doc_pref = re.sub(r"[(（][^)）]*(?:우선|우대)[^)）]*[)）]", "", doc)   # 우대 괄호군 통째 제거
        doc_pref = re.sub(r"(?:수급자?|차상위)\s*(?:우선|우대)", "", doc_pref)  # 괄호 밖 우대 표현
        doc_pref = re.sub(r"미수급[가-힣]*", "", doc_pref)                     # 부정어(소득 무관)
        if "차상위" in doc_pref:
            ceil = 50
        elif any(k in doc_pref for k in ["기초생활", "생계급여", "기초생활수급", "기초수급", "수급자", "수급 가구", "급여 수급"]):
            ceil = 50
    return ceil
...
def _lookup_guide_template(name: str) -> dict:
    """정확 이름 대신 접두 매칭 — '실업급여 (구직급여)'가 '실업급여' 템플릿과 안 맞아 dead였던 문제 해소.
    (프론트 welfare-engine.ts generateGuides 와 동일 규칙 — tests/test_engine_parity.py 가 고정)"""
    if name in _GUIDE_TEMPLATES:
        return _GUIDE_TEMPLATES[name]
    for k in _GUIDE_TEMPLATES:
        if name.startswith(k):
            return _GUIDE_TEMPLATES[k]
    return {}
...
def mock_guides(policies: list[dict]) -> dict:
    ...
    for p in [ep for ep in policies if ep.get("eligible")][:5]:
        name = p.get("name", "")
        pid = p.get("id", "")
        tmpl = _lookup_guide_template(name)
        full = _POLICY_MAP.get(pid, {})
        guides.append({
            "policy_id": pid,
            "policy_name": name,
