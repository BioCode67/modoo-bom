...
![tests](https://img.shields.io/badge/frontend_tests-728_passed-brightgreen) ![pytest](https://img.shields.io/badge/backend_pytest-130%2B_passed-brightgreen) ![e2e](https://img.shields.io/badge/E2E_실브라우저-6스위트·에러0-brightgreen) ![data](https://img.shields.io/badge/실데이터-5,300여건_검증-blue) ![lighthouse](https://img.shields.io/badge/Lighthouse-A11y100·BP100·SEO100-blue)
...
- **🚀 데스크탑 원클릭 자동화 (대회 하이라이트)** — '전부 자동발급 + 자동신청까지' 한 번으로: 정부24 서류 순차 발급(📱 인증 승인만 본인) → 복지로 신청 양식 자동작성 + **방금 발급물 자동첨부** → **제출 직전 정지**(제출은 본인 확인). 진행률 바·⏭ 단계 건너뛰기·📱 인증 알림음·🔁 새로고침 이어보기·발급 전 원버튼 점검·🗂 내 서류함·📷 촬영 등록 — 전부 실브라우저 E2E로 회귀 고정.
...
품질: ESLint(0) · 단위 테스트(vitest 728 + pytest 130+) · 실브라우저 E2E 6스위트(웹·데스크탑·모바일·촬영·흐름·접근성) · TypeScript · ErrorBoundary · 주요 복지 금액 2026년 공식 출처 검증
...
정부24·복지로·건강보험공단·고용24·국민연금·장학재단을 에이전트가 직접 조작해 서류 발급(데스크탑 앱 15종·확장 13종)·복지 신청을 진행합니다. 세 가지 방법:
...
└── frontend/                        # 메인 — 백엔드 없이 동작 (정적 배포)
    ...
    │   └── src/**/*.test.ts         # vitest 728 (엔진·검색·모니터링·docScan·authCue …)
...
> 이 섹션은 팀원이 기획서·발표자료를 만들 때 필요한 사실을 모두 담습니다. 전 수치 2026-07 실측.
...
챗봇이 아니라 인지→판단→이해→행동→관찰의 순환 전체가 에이전트다. (다이어그램: `기획서자료실/02-다이어그램/02-AI에이전트-루프.png`)
...
- **품질 게이트**(차트 4): 프론트 vitest 728 · 백엔드 pytest 130+ · 실브라우저 E2E 6스위트(웹 10여정·데스크탑 16종·모바일·촬영·흐름·접근성 axe 0
...
| 🚀 원클릭 연쇄(발급→자동첨부→신청 대기) | 클릭 1번 + 인증 승인만 | `npm run e2e:desktop` (16종 회귀) |
...
- **수치 갱신**: 데이터·테스트 수가 바뀌면 기획서자료실의 차트·인포그래픽도 함께 갱신한다(소스: `make_charts.py`, `diagrams.html`).
