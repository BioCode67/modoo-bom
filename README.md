...
회원가입·백엔드 없이 바로 동작합니다. 프론트엔드는 **전국 5,185건 복지 정책**(보건복지부 검증 시드 124건
+ 한국사회보장정보원 공공데이터 5,061건)과 자격 판정 엔진, 그리고 **브라우저에서 직접 도는 AI**를 실행하므로 즉시 로딩되고 항상 켜져 있어요.
...
**품질**: ESLint(0) · 단위 테스트(vitest 168 + pytest 13) · TypeScript · ErrorBoundary · 주요 복지 금액 2026년 공식 출처 검증
