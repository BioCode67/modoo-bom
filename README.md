...
품질: ESLint(0) · 단위 테스트(vitest 772 + pytest 254) · 실브라우저 E2E 12스위트(웹·데스크탑·모바일·촬영·흐름·접근성) · TypeScript · ErrorBoundary · 주요 복지 금액 2026년 공식 출처 검증
...
└── frontend/                        # 메인 — 백엔드 없이 동작 (정적 배포)
    ...
    │   └── src/**/*.test.ts         # vitest 772 (엔진·검색·모니터링·docScan·authCue …)
...
- **품질 게이트**(차트 4): 프론트 vitest 772 · 백엔드 pytest 254 · 실브라우저 E2E 12스위트(웹 10여정·데스크탑 29종·모바일·촬영·흐름·접근성 axe 0
...
| 🚀 원클릭 연쇄(발급→자동첨부→신청 대기, 🔑 로그인 인증 1회·일시 오류 자동 재시도) | 클릭 1번 + 인증 승인만 | `npm run e2e:desktop` (29종 회귀) |
