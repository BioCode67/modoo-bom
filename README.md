...
- **외국인 딥퍼널 다국어 UI** — 외국어로 검색하면 정책 상세·신청 키트의 UI 골격이 자국어로(en·vi·zh·ja·th·ru·ar) + 통역 배너(전화 1577-1366 다누리). 정책 본문은 한국어 유지(기계번역·환각 없음)
- **🪪 창구 도움 카드** — 말이 안 통해도 주민센터 창구에서 '보여주기'로 신청 시작: 본인용 자국어 안내(검증된 정적 사전) + 직원이 읽을 큰 한국어 문장 + 필요서류 체크리스트 + 통역 연락처(아랍어는 다누리 미지원이라 129만 — 정직 안내). 큰 글씨 인쇄 지원
...
- **혁신** — 생애주기 시뮬레이터(미래 받을 복지), 긴급복지 빠른 진단(위기 상황), 우리 가족 복지 한눈에(가구 단위), 결과 공유(이미지 카드 + 📋 카톡용 '글로 복사' — 어르신→자녀 도움 요청 경로), 신청 사유서·위임장 대필(규칙 기반·환각 0), 가까운 주민센터 찾기(지도)
...
품질: ESLint(0) · 단위 테스트(vitest 647 + pytest 130) · 실브라우저 E2E 10여정 · TypeScript · ErrorBoundary · 주요 복지 금액 2026년 공식 출처 검증 · **5차례 다중에이전트 적대 감사로 자격판정·정직성·접근성·PII·보안(CSRF·주입)·성능 결함 ~68건 정정**
...
정부24·복지로·건강보험공단·고용24·국민연금·장학재단을 에이전트가 직접 조작해 서류 발급(데스크탑 앱 15종·확장 13종)·복지 신청을 진행합니다. 두 가지 방법:
