...
민간은 전 항목 공식 사이트 실측 검증, 이름 기준 병합·디듑)와 자격 판정 엔진, 그리고 **브라우저에서 직접 도는 AI**를 실행하므로 즉시 로딩되고 항상 켜져 있어요.
(Claude 키와 FastAPI 백엔드를 연결하면 LangGraph 에이전트·RPA 자동발급 등 고급 기능이 활성화됩니다.)
...
- 🤖 **행동형 챗 에이전트** — 프로필을 알고 답하고("내가 뭐 받을 수 있어?" → 자격+이유), **대화 맥락 기억**("그거/다 담아줘" 한마디로 관심목록 저장), 열면 **급한 마감·서류부터 브리핑**. 전부 규칙엔진(서버·LLM 불필요, 환각 없음)
- 🌱 **대화형 온보딩 "새싹이와 대화"** — 폼 대신 마스코트가 한 번에 하나씩 묻고 **탭으로만** 답하는 프로필 입력. 어르신(65+) 선택 시 **큰글씨 원탭 제안**, 질문 **음성 읽어주기**(TTS), 새로고침해도 이어서(임시저장)
- 🌐 **외국인 딥퍼널 다국어 UI** — 외국어로 검색하면 정책 상세·신청 키트의 UI 골격이 자국어로(en·vi·zh·ja·th·ru·ar) + 통역 배너(☎1577-1366 다누리). 정책 본문은 한국어 유지(기계번역·환각 없음)
...
- 🔎 **확인/조회** — 1분 프로필 위저드 + 자격 판정(2026 정밀 선정기준), 5,250여 종 정책 탐색·검색, **대화형 AI 상담 챗봇**(연령·상황·소득으로 좁혀 추천)
...
**품질**: ESLint(0) · 단위 테스트(vitest 353 + pytest 34) · 실브라우저 E2E 10여정 · TypeScript · ErrorBoundary · 주요 복지 금액 2026년 공식 출처 검증
**📁 팀원용 기획서 자료실**: [`기획서자료실/`](기획서자료실/) — 인포그래픽·다이어그램 8종·통계차트 6종·스크린샷 14장·경쟁비교표·**PPT 55페이지 구성안**까지 전부 (아래 [프로젝트 상세 문서](#-프로젝트-상세-기획서용-전체-문서)와 함께 사용).
**기획서 자료**: `docs/기획서자료/`에 아키텍처·AI흐름·RPA·데이터 다이어그램 + 차트 + 한 장 요약 포스터 제공.

### 🤖 자동 서류발급 · 자동 신청

정부24·복지로·건강보험공단·고용24·국민연금·장학재단을 **에이전트가 직접 조작**해 서류 발급(13종)·복지 신청을 진행합니다. 두 가지 방법:

**① 크롬 확장 (권장 — 서버 없이, 배포 사이트에서 바로)**
...
**정부 공식 공개데이터**로 계속 갱신·확장할 수 있어요. 프론트가 `public/policies.json`을 런타임에 자동 병합하므로 **코드 수정 없이** 늘어납니다.
...
> 이 섹션은 팀원이 기획서·발표자료를 만들 때 필요한 **모든 사실**을 담습니다. 전 수치 2026-07 실측.
> 이미지·차트는 [`기획서자료실/`](기획서자료실/)에 완성본이 있습니다 — 숫자를 다시 타이핑하지 말고 이미지를 쓰세요.

## 1. 배경 — 왜 이 문제인가

한국 복지는 **신청주의**: 자격이 있어도 신청해야 받는다. 그런데,
...
→ 결론: 필요한 것은 '더 나은 검색'이 아니라, 사용자를 **대신해 주는 에이전트**다.

## 2. 솔루션 — AI Agent 자율 루프

챗봇이 아니라 **인지→판단→이해→행동→관찰**의 순환 전체가 에이전트다. (다이어그램: `기획서자료실/02-다이어그램/02-AI에이전트-루프.png`)

| 단계 | 무엇을 하나 | 구현 |
|---|---|---|
| 인지 | 새싹이와 대화 온보딩(탭만으로 입력) · 자연어 한 문장 · 음성 | `MascotChat` + `onboardingFlow` + `parseQuery` (규칙 NLU, 환각 0) |
| 판단 | 5,251건에서 자격판정(2026 공식 선정기준: 생계32·의료40·주거48·교육/차상위50%) · 확실/관련 신뢰도 분리 · **모집종료·인구불일치·소득초과 하드 게이트** | `welfare-engine.ts` (백엔드 `mock_responses.py`와 패리티) |
| 이해 | 쉬운 말 결과 · 용어사전 · **7개 언어 AI 답변**(EN·VI·ZH·JA·TH·RU·AR) · TTS 읽어주기 | `aiAnswer.ts` + `glossary` + `useTTS` |
| 행동 | 챗 "다 담아줘"→실제 저장 · 서류 13종 자동발급 · 복지 6종 자동신청(RPA) · 원탭 신청(정보복사+공식페이지) | `chatAgent` + 크롬 확장 + `quickApply` |
...
## 3. 데이터 — 5,251건 전부 실데이터

(차트: `기획서자료실/03-통계차트/chart1·2`)

| 출처 | 건수 | 검증 방식 |
|---|---|---|
| 지자체 복지(LOC-) | 4,598 | 한국사회보장정보원 OpenAPI(B554287) — 지역배지·시군구 2차 필터 |
...
| 청년주택 공고(HOU-) | 7 | LH·SH·GH 등 **살아있는 공고 게시판**으로 연결(가짜 공고 미생성) |
...
(다이어그램: `03-시스템-아키텍처.png` · `04-데이터-파이프라인.png` · `09-LangGraph-10노드.png`)
...
2. **백엔드(선택, 감지 시 자동 활성)** — FastAPI+WebSocket, LangGraph 10노드 StateGraph, Claude LLM(지식형 챗), ChromaDB RAG.
...
**하이브리드 챗**(다이어그램 06): 행동·개인화=로컬 에이전트(즉시·정확), 지식 질문=Claude(🧠 라벨, 12초 타임아웃), 실패=규칙 폴백.
...
## 7. 대회 정보·일정 (2026 AI·SW중심대학 디지털 경진대회 SW부문)

- 주제: **"라이프스타일 혁신을 위한 AI Agent 기반 SW 개발"** · 본선 8/11(화)
- 예선 = **투표 100%**(전문가 70+참가자 30) · 본선 = 창의성30·구현완성도25·**사용자편의성20**·주제적합성15·사용가능성10
- 제출: 기획서 PDF **7/7(화) 10:00** → 최종 산출물 PDF 7/15(수) → (후보)팀 코드 7/20(월) → 본선자료 7/31(발표자료는 PDF만)
- 상세 일정·할 일: [`docs/사용자-할일.md`](docs/사용자-할일.md)

## 8. 팀원 작업 가이드

- **기획서/PPT**: [`기획서자료실/README.md`](기획서자료실/README.md)부터 — 55페이지 구성안에 슬라이드별 자료 매핑 완료.
- **DACON 제출본**: `docs/제출/모두봄-예선산출물-작성본.pptx`(공식 양식에 원고+실화면 삽입 완료 — 팀정보 3칸만 입력).
- **수치 갱신**: 데이터·테스트 수가 바뀌면 Claude에게 "기획서자료실 재생성" 요청(소스: `make_charts.py`, `diagrams.html`).
- **로컬 실행/배포**: 위 '실행 방법' 절 · 재배포는 `cd frontend && npm run deploy`.
