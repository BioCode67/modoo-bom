...
## 산출물 당일 배치 — 심사 동선 결함 수정 (2026-07-15)

배포는 이제 **main 푸시 → GitHub Actions(deploy.yml) 자동 빌드·gh-pages 배포**다(과거 '토큰 workflow 스코프 불가' 표기는 옛말 — 사용자가 웹 업로드로 워크플로 추가, run 이력 전부 success). `npm run deploy` 수동 경로도 여전히 동작.
- **신청 자동이동 실버그 수정(사용자 신고)**: seed 정책은 application이 자유텍스트라 '신청'이 정부24 키워드
  검색으로 착지(청년월세지원 실측). → `KNOWN_APPLY_URLS` 22종 확장(공공데이터 B554287의 검증
  wlfareInfoId 대조: 청년월세 WLF00004661 등) + `borrowApplyUrl`(카탈로그 교차참조 — 같은 이름의 공공데이터
  정책 검증 딥링크 재사용, 정확/접두 70% 매칭만 허용해 '아동수당→장애아동수당' 오매칭 차단, 배열참조 캐시).
  gov.kr 검색은 최후 폴백으로 강등. `scripts/check-links.mjs` BOKJIRO 목록 동기화(총 27 ID).
- **다국어 AI 검색 결과 온디바이스 번역**: 외국어 질의 시 결과 카드(정책명·대상·혜택·분류)를 브라우저 내장
  Translator API(Chrome/Edge 138+)로 질의 언어로 표시(`lib/onDeviceTranslate.ts`, 서버 전송 없음).
  미지원 브라우저는 한국어 원문+Google 번역 링크 폴백. 카드에 '자동 번역' 배지 — 원문(신청·자격 기준)은
  한국어 명시(정직성). 금액·자격 로직은 항상 한국어 원문 기준. Hero 다국어 예시 클릭(사용자 제스처)에서
  번역기 워밍업. ⚠️ 샌드박스는 외부망 차단이라 모듈 로직만 실브라우저 목킹 검증 — 실번역 화면은 실크롬 확인 필요.
- **히어로 줄간격**: 기존 `leading-[1.24]`가 `sm:text-5xl`의 line-height:1에 덮여 데스크탑 무효였음 →
  `!leading-[1.35]`로 실적용(실측 1.0→1.35).
- 검증: vitest **635**(신규 14 포함) · tsc · lint 0 · build · 배포본 번들에 WLF00004661 포함 실측.
...
## 마감 전 집중 배치 — w2 (2026-07-11)

7/15 산출물·7/17 투표(참가자+커뮤니티가 기획서·영상·데모링크 보고 투표) 대비, 투표자 첫인상과 안정성 위주.
- 히어로 말풍선이 3D 새싹 얼굴을 가리던 문제 수정(흐름 배치 전환) + 2D/3D 마스코트 통일(흙·하이라이트·
  웃는 입 — 3D 입이 구 안에 묻혀 안 보이던 것 z 보정, 실브라우저 스크린샷 검증) + 파비콘 정합.
- 챗: 비복지 질문에도 다정히 답 후 복지로 연결(새싹이 정체성 — 라이브 실측), 콜드스타트 중 질문이 규칙
  폴백으로 침묵하던 갭 수정(웨이크 대기 25초 캡). Render는 main 푸시 자동배포임을 실측 확인.
- 하이브리드 검색(RRF: 온디바이스 임베딩+키워드 개념 랭킹) 신설 → 적대 리뷰가 치명결함 발견:
  영어 관사 a/I가 1글자 예외로 정책명 라틴 표기(AI·MRI)에 오탐. 근본 수정(1글자 예외 한글 한정)+
  키워드 레인 한글 게이트. 라이브 실측 '기초연금' 1위·영어 한부모 질의 잡음 0.
- 감사 지적 전건 처리: --no-sandbox 번들 한정(실브라우저 샌드박스 유지)·진행중 태스크 퇴거 방지·
  여정 큐 백프레셔(queued_slot)·status 스크린샷 PII 토큰 게이트(양쪽 앱 패리티+프론트 폴러 ?t=).
- 릴리스: 데스크탑 exe 재빌드·app-v0.3.0 자산 교체(친절 배너+오늘 수정 전부, REST API — gh CLI 없음,
  git credential 토큰). 웹 '앱 바로 받기'가 exe 직접 다운로드로 연결.
- 임베딩 재생성 5,308건(필드예산 패시지), 미사용 의존성 정리(radix 6종 등·langchain-community),
  문서 문체 정리(공개 README·내부 문서), 제출 원고 수치 최신화·게시글 초안·시연영상 90초 촬영대본.
- 품질: 프론트 563 · 백엔드 84+(lean) · E2E 스모크+챗/저장/서민금융/로컬앱 전부 green · 딥링크 19종 정상.
- 남은 것: 시연영상 촬영(사람), rate_limit XFF(클라우드·추후), 본선 자료.
