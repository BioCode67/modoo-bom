...
## 1-B. 장학금(기업·민간재단·지자체·대학) — 한국장학재단 학자금지원정보
> 정부 복지 DB(한국사회보장정보원)에는 **국가/지자체 복지**만 있고 **기업·민간재단 장학금**은 없다.
> 이를 실데이터로 보강하는 별도 공식 소스.

| 항목 | 내용 |
|---|---|
| 원천 | 공공데이터포털 **한국장학재단_학자금지원정보(대학생)** (data.go.kr/data/15028252) |
| 포함 | 전국 **지자체·민간장학재단·대학·기업** 장학금 약 **1,850건** |
| 형식 | CSV(로그인 불필요 다운로드). ⚠️ 파일 다운로드는 data.go.kr 세션 게이트로 **헤드리스 자동수집 불가** → 사용자가 브라우저에서 1회 다운로드 |
| 컬럼 | 운영기관명·상품명·**운영기관구분(지자체/민간재단/대학 등)**·지원내역·성적/소득/특정자격 기준·제출서류·**홈페이지(실제 신청 URL)**·모집시작/종료일 |
| 정규화 | `org_type`(기업/민간/지자체 배지·필터용)·`deadline`·실제 `application` URL·`is_cash=true`·금액 파생 |

### 반영 방법 (사용자 1회 작업)
```bash
# 1) 브라우저에서 CSV 다운로드(로그인 불필요):
#    https://www.data.go.kr/data/15028252/fileData.do → [다운로드]
# 2) 병합(기존 카탈로그 보존, 이름 기준 디듑):
cd backend && python etl/ingest_welfare.py --kosaf ~/Downloads/한국장학재단_학자금지원정보*.csv
# 3) 프론트 반영:
cd ../frontend && npm run deploy
```
→ 파서·병합·금액/조건 파생은 구현·검증 완료(`ingest_kosaf_csv`). CSV만 넣으면 즉시 1,800여 건이 붙는다.
